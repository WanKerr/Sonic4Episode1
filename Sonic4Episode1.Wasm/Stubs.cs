...
using static SDL2.SDL.SDL_MessageBoxFlags;
using static SDL2.SDL.SDL_MessageBoxButtonFlags;
...
namespace Microsoft.Xna.Framework.GamerServices
{
    ...
    {
        Alert = SDL_MESSAGEBOX_INFORMATION,
    ...
    public static partial class Guide
    {
        ...
        // (crying)
        [JSImport("Guide_BeginShowMessageBox", "main.js")]
        internal static partial void Guide_BeginShowMessageBox(int playerIndex,
            string title,
            string text,
            string[] buttons,
            int focusButton,
            int icon,
            [JSMarshalAs<JSType.Function<JSType.Number>>] Action<int> callback);
        ...
        public static IAsyncResult BeginShowMessageBox(PlayerIndex player, string title, string text, IEnumerable<string> buttons, int focusButton, MessageBoxIcon icon, AsyncCallback callback, object state)
        {
...
#if WASM
            IsVisible = true;
            Guide_BeginShowMessageBox((int)player, title, text, buttons.ToArray(), focusButton, (int)icon, (btn) =>
            {
                if (btn == -1)
                {
                    tcs.SetResult(null);
            ...
            var data = new SDL_MessageBoxData
            {
                title = title,
                message = text,
                numbuttons = btns.Length,
                buttons = new SDL_MessageBoxButtonData[btns.Length]
            };
            ...
            for (int i = 0; i < btns.Length; i++)
            {
                ...
                if (i == focusButton)
                    data.buttons[i].flags = SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT;
            }

            data.window = IntPtr.Zero;
            data.flags = (SDL_MessageBoxFlags)icon;

            IsVisible = true;

            if (SDL_ShowMessageBox(ref data, out int button) == 0)
            {
...
namespace Microsoft.Devices.Sensors
{
    public partial class Accelerometer
    {
        ...
        public event EventHandler<SensorReadingEventArgs<AccelerometerReading>> CurrentValueChanged;
        ...
        public void Stop()
        {
            Accelerometers.Remove(this);
        }

        private static List<Accelerometer> Accelerometers { get; } = [];

        [JSExport]
        internal static void Invoke(float x, float y, float z)
        {
            foreach (var item in Accelerometers)
            {
                item.CurrentValueChanged?.Invoke(item, new(new() { Acceleration = new Vector3(x, y, z) }));
            }
        }
    }
    ...
    {
        public readonly T SensorReading;

        internal SensorReadingEventArgs(T reading)
        {
...
{
    public partial class WebBrowserTask
    {
#if WASM
        [JSImport("WebBrowserTask_Open", "main.js")]
        internal static partial void WebBrowserTask_Open(string url);
#endif
...
#elif WASM
            WebBrowserTask_Open(Uri.ToString());
#else
