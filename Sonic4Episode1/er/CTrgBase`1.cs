        ...
        public virtual void Update()
        {
            ...
            for (int index = 0; index < AppMain._am_tp_touch.Length; ++index)
            {
                ...
                if (AppMain.amTpIsTouchOn(index))
                {
                    ...
                    if (is_edge)
                        this.m_pos[index] = new IntPair(amsTpTouchStatus.push[0], amsTpTouchStatus.push[1]);
                    IntPair pos = new IntPair(amsTpTouchStatus.on[0], amsTpTouchStatus.on[1]);
                    if (!this.m_flag[1])
                ...
                else
                {
                    ...
                    move = new IntPair(amsTpTouchStatus.pull[0], amsTpTouchStatus.pull[1]) - this.m_pos[index];
                    if (is_edge)
